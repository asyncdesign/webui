...
(function(webui, ui, $, undefined) {
    ...
    var dependsOnRegExpMatches = function(dependsOnSelector, dependsOnRegExp) {
        return isTextbox(dependsOnSelector) && dependsOnRegExp.test($("#" + dependsOnSelector).val()) || isTextarea(dependsOnSelector) && dependsOnRegExp.test($("#" + dependsOnSelector).text()) || isSelect(dependsOnSelector) && dependsOnRegExp.test($("#" + dependsOnSelector + " option:selected").text()) || isCheckbox(dependsOnSelector) && dependsOnRegExp.test($("#" + dependsOnSelector).is(":checked"));
    };
    ...
    var notificationDuration = 5e3;
    var notificationFadeInDuration = 600;
    var notificationFadeOutDuration = 300;
    var showNotification = function(message, dynamic, type) {
        try {
            var notificationContainer = !$(".notification-container").length ? $("<div></div>").addClass("notification-container").addClass("notification-" + notificationPosition).appendTo("body") : $(".notification-container");
            var notificationItemOuter = $("<div></div>");
            var notificationItemInner = $("<div></div>").hide().addClass("panel notification-item notification-type-" + type).appendTo(notificationContainer).animate({
                opacity: "show"
            }, notificationFadeInDuration).wrap(notificationItemOuter);
            ...
            var notificationItemClose = $("<div></div>").addClass("notification-icon-cancel move-right").appendTo(notificationItemHeaderColumn).html("").click(function() {
        ...
        try {
            ...
            if (arguments.length > 3 && revertOnClick) {
                $("#" + selector).click(function() {
                    if ($("#" + selector).val().length === 0 && (isTextbox(selector) || isPassword(selector) || isTextarea(selector)) || isCheckbox(selector) || isSelect(selector) || isDiv(selector) || isSpan(selector)) {
    ...
    ui.initialiseSelect = function(selector, itemCount, enabledCssClass, disabledCssClass, enabledValue, disabledValue) {
        try {
            if (arguments.length === 1) {
                var totalOptions = $("#" + selector + " option").length;
                if (totalOptions < 2) {
    ...
    ui.hideNotification = function(notification, dynamic) {
        try {
            notification.animate({
                opacity: "0"
            }, dynamic ? notificationDuration : notificationFadeOutDuration, function() {
                notification.parent().animate({
                    height: "0px"
                }, notificationFadeOutDuration, function() {
