...
(function(win) {
    var _ui = win.ui, _webui = win.webui, root = document, isObject = function(el) {
        ...
        if (el.length === 1) {
            return isTextbox(el[0]) && el.val() === value || isTextarea(el[0]) && el.text() === value || isSelect(el[0]) && el.find("option:checked").text() === value || isCheckbox(el[0]) && el.is(":checked") === value || isRadio(el[0]) && el.is(":checked") === value;
        }
        return false;
        ...
        if (toggleContainer.length) {
            ...
            var transitionDelay = toggleContainer.data("transition-delay");
            ...
            if (toggleItem.length) {
                ...
                if (offCanvas && toggleBody.length) {
                    ...
                    if (toggleItem.hasClass("off-canvas-closed")) {
                        ...
                        delay(transitionDelay, function() {
    ...
    webui.calculateCursorSpeed = function(e, previousEvent) {
    ...
    /* EVENT METHODS */    webui.on = function(name, callback) {
...
(function(win) {
    ...
    /* PUBLIC */    Object.defineProperty(webui.prototype, "navbarControl", {
...
(function(win) {
    /* PRIVATE */
    var zoom = 1.05, trigger = "hover", transitionDuration = 500;
    /* PUBLIC */    Object.defineProperty(webui.prototype, "zoomControl", {
        value: function(options) {
            var settings = ui.extend({
                zoom: 1.05,
                tringer: "hover",
    ...
    var CarouselInstance = function(carousel, settings) {
        var interval = settings.interval, autoPlay = settings.autoPlay, autoScale = settings.autoScale, playDirection = settings.playDirection, stopOnHover = settings.stopOnHover, transitionDuration = settings.transitionDuration, transitionType = settings.transitionType, transitionOrientation = settings.transitionOrientation, width = settings.width, height = settings.height, carouselHolder, carouselItems, carouselItemCount, carouselItemWidth = 0, carouselItemHeight = 0, itemBorderWidth = 0, itemBorderHeight = 0, current = 1, cycle = false, delta = 1, transitionCompleted = true, run = null, shift = function(dir) {
            ...
            if (transitionCompleted) {
                ...
                if (transitionType === "fade") {
                    carouselHolder.fadeOut(transitionDuration, .9, function(element) {
                        element.slideHorizontal("right", carouselItemWidth, 0, function(element) {
                            shift("left");
                            element.fadeIn(transitionDuration, .3, function(element) {
